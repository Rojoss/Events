...
public enum Equipment {
    SPECTATOR(new CWItem[] {
            ...
            new CWItem(Material.REDSTONE_BLOCK).setSlot(8).hideTooltips().setName("&4&lLEAVE").setLore(new String[] {
                    "&7Stop spectating the current game.", "&7You will be teleported back to the lobby!"
            })
    }),
    LOBBY(new CWItem[]{
            new CWItem(Material.EYE_OF_ENDER).setSlot(0).hideTooltips().setName("&6&lJOIN A GAME").setLore(new String[] {
                    "&7Select a game and map to play!", "&7You can also join games by using the signs in the lobby!"
            }),
            new CWItem(Material.BOOK).setSlot(2).hideTooltips().setName("&5&lPREFERENCES").setLore(new String[] {"&7Customize your favourite game modifiers!"}),
            new CWItem(Material.PAPER).setSlot(4).hideTooltips().setName("&d&lSETTINGS").setLore(new String[] {"&7Modify your game settings!"}),
            new CWItem(Material.NETHER_STAR).setSlot(6).hideTooltips().setName("&a&lSTATS").setLore(new String[] {"&7Display statistics!"}),
    });
